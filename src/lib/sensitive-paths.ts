...
export function bashReferencesEnvPath(command: string): boolean {
  ...
  for (const raw of tokens) {
    ...
    try {
      ...
      if (isEnvSecretsFileName(path.basename(resolved))) return true;
    } catch {
      // ignore
    }
    if (isEnvSecretsFileName(path.basename(t))) return true;
  }

  // Fallback: path-like ".env" without being caught as a single token
  if (/(?:^|\s|[/\\([(=])\s*\.env(?:\.|$|[\s'"\`])/.test(s)) return true;
  if (/[/\\]\.env(?:\.|$|[\s'"\`])/.test(s)) return true;
