  ...
  if (minimized) {
    return (
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-1 rounded-lg border border-yellow-500/30 bg-black/90 px-2 py-1 shadow-lg backdrop-blur">
        ...
        <button onClick={() => setMinimized(false)} className="p-0.5 text-yellow-500/70 hover:text-yellow-500">
          <Bug className="size-3" />
        </button>
        <button onClick={() => setOpen(false)} className="p-0.5 text-yellow-500/70 hover:text-yellow-500">
          <X className="size-3" />
        </button>
      </div>
    );
    ...
    <div className="fixed bottom-4 right-4 z-50 w-[480px] max-h-[600px] flex flex-col rounded-lg border border-yellow-500/30 bg-black/90 shadow-lg backdrop-blur overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-yellow-500/20">
        ...
        <div className="flex gap-1">
          <button onClick={() => setMinimized(true)} className="p-0.5 text-yellow-500/70 hover:text-yellow-500">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {Object.entries(data).map(([key, value]) => (
          <div key={key}>
            <div className="text-[10px] font-medium text-yellow-500/70 uppercase tracking-wider mb-0.5">{key}</div>
